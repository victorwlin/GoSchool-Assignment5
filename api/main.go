...
func getFriends(w http.ResponseWriter, r *http.Request) {
	...
	results, err := db.Query("SELECT * FROM kit_db.victor")
	if err != nil {
		panic(err.Error())
	}
...
func friendFunc(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		if friendExists(db, friendParam) {
			results, err := db.Query("SELECT * FROM kit_db.victor WHERE friend_name = '" + friendParam + "' LIMIT 1")
			if err != nil {
				panic(err.Error())
			}
	...
	if r.Header.Get("Content-type") == "application/json" {
		...
		if r.Method == "POST" {
			...
			if err == nil {
				...
				// check if friend exists
				if !friendExists(db, friendParam) {
					_, err := db.Query("INSERT INTO kit_db.victor VALUES ('" + newFriend.FriendName + "', '" + newFriend.Group + "', " + strconv.Itoa(newFriend.DesiredFreq) + ", '" + newFriend.LastContact + "')")
					if err != nil {
						panic(err.Error())
					}
		...
		if r.Method == "PUT" {
			...
			if err == nil {
				...
				// check if friend exists
				if friendExists(db, friendParam) {
					query := fmt.Sprintf("UPDATE kit_db.victor SET `group`='%s', `desired_freq`=%d, `last_contact`='%s' WHERE `friend_name`='%s'", editFriend.Group, editFriend.DesiredFreq, editFriend.LastContact, editFriend.FriendName)
					_, err := db.Query(query)
					if err != nil {
						panic(err.Error())
					}
...
func friendExists(db *sql.DB, friend string) (exists bool) {
	search, err := db.Query("SELECT EXISTS(SELECT * FROM kit_db.victor WHERE friend_name = '" + friend + "')")
	if err != nil {
		panic(err.Error())
	}
